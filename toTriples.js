...
if (arguments.length > 0) {
	...
	fs.readFile("profile.firenze.js", "UTF-8", function(err, data) {
		if (err)
			throw err;
		start(JSON.parse(data));
	});
...
function start(jConfig) {
	...
	/* lista dei package */
	http.get(jConfig.packagesCsvUrl, function(res) {
		...
		res.on('end', function() {
			...
			csv().from(resText, {
				...
				if (jConfig.groupBy) {
					/*
					 * rimetto insieme nella mappa di ogni dataset quello che
					 * risulta essere la distinct nel csv
					 */
					var theMainField = jConfig.groupBy['mainField'];
					var tempResJ = rows[resJ[theMainField]];
					var singleGroup = {};
					$.each(jConfig.groupBy['fields'], function(key, value) {
						singleGroup[value] = resJ[value];
					});
					if (tempResJ) {
						var tempSingleGroup = rows[resJ[theMainField]][jConfig.groupBy['groupName']];
						tempSingleGroup.push(singleGroup);
						resJ[jConfig.groupBy['groupName']] = tempSingleGroup;
						rows[resJ[theMainField]] = resJ;
					} else {
						resJ[jConfig.groupBy['groupName']] = [ singleGroup ];
						rows[resJ[theMainField]] = resJ;
					}
				} else {
					singleDataset(resJ, jConfig);
				}
				...
				if (jConfig.groupBy) {
					...
					$.each(rows, function(key, value) {
						singleDataset(value, jConfig);
						conta++;
					});
					console.log("extracted " + conta + " datasets");
				}
			 ...
			 * singleDataset(resArray[int]); // return; }
			 */
		});
	}).on('error', function(e) {
	...
	/* singolo package */
	function singleDataset(resJ, jConfig) {
		/* response in json */
		var map = jConfig.datasetMap;

		/* genero la uri della nuova risorsa */
		var resourceUri = generateUri($.trim(resJ[jConfig.resourceUri.field]), jConfig.resourceUri);
		console.log('processing ' + resourceUri);
